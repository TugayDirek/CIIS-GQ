...
def drawStructure(tetrad_and_loop, pdb_name,ONZ,tetrad_onz, atoms_sequence, other_structures):
    ...
    global_G=10000
    bulge_bases = []
    other_structures_dict = {}

    if(len(other_structures[pdb_name].keys())>0):
        for bond in other_structures[pdb_name].keys():
            #print("////////",bond)
            elements = bond.split(",")
            other_structures_dict[elements[1]] = [elements[3],elements[4]]
    ...
    for tetrads in tetrad_onz[pdb_name]:
        ...
        for tetrad in tetrads:
            ...
            if(len(other_structures_dict.keys())>0):
                l1 = other_structures_dict.keys()
                if (str(tetrad) in l1):
                    tetrad_list = tetrad_list + "(" + str(other_structures_dict[str(tetrad)][0]) + "),"
    ...
    for element in loop:
        ...
        for element2 in element:
            ...
            if(len(other_structures_dict.keys())>0):
                l1 = other_structures_dict.keys()
                if (str(g_number) in l1):
                    for a in range(10):
                        if a%2==0:
                            pen.penup()
                        else: pen.pendown()
                        pen.setpos(x+3*a,y_positions[foundG4s[int(element2)]]+a)
                    pen.pendown()
                    pen.color("gray")
                    pen.write(str(other_structures_dict[str(g_number)][0])+str(other_structures_dict[str(g_number)][1]), font=("Courier", 10, "bold"))
                    pen.color("black")
                    pen.penup()
                    pen.setpos(x,y_positions[foundG4s[int(element2)]])
                    pen.pendown()
    ...
    for filename in files:
        ...
        with open(filename, 'r') as f:
            ...
            for line in f:
                ...
                ############### deleted since it only benefits run time but adds complexity to the code
                ...
                # if (
                #         # (line[18:20].__contains__("G") or line[18:20].__contains__("FL") or line[18:20].strip().__eq__(
                #         # "F2") or line[17:20].strip().__eq__("GF0"))
                #         # and
                #
                #         any(("N1") in x for x in coordinatesDict.keys()) and any(
                ...
                if ((list[0].__contains__("ATOM") or list[0].__contains__("HETATM")) and
                    ############ deleted since the coordinates of bases other than guanines were needed after revision
                    #     (
                    ...
                    # "0G") ) and
                        typeOfNucleicAcid.__contains__("DNA") and atomsDNA.__contains__(pdbName)):
                    #print(line)
                    #print("///////////////////////", line[13:15].strip(),"///////",line[18:20].strip())
                    atomsDNA_sequence.get(pdbName)[line[23:27].strip()] = line[18:20].strip()
                    # print("4", atomsDNA.get(pdbName))
                    #
                    # atomsDNA.get(pdbName)[line[23:27].strip() + "," + line[18:20].strip()] = line[18:20].strip()   ####################
                    # print("3", atomsDNA.get(pdbName))
                    ...
                    "N7") or  line[13:15].strip() == ("N3") or  line[13:15].strip() == ("N6") or line[13:15].strip() == ("N9") or line[13:15].strip() == ("N4")
                            or line[13:15].strip() == ("O6") or line[13:15].strip() == ("O4") or line[13:15].strip() == ("O2") ) and (list[0].__contains__("ATOM") or (list[0].__contains__("HETATM") and (line[18:20].strip().__eq__("F2") or line[18:20].strip().__eq__("FL") ) ))) :
                        #print(line[17:20].strip(), line)
                        #print(atomsDNA.get(pdbName))
                        ...
                        #print("---",coordinatesDict,line[23:27].strip() + "," + line[18:20].strip(),pdbName,"----")

                        atomsDNA.get(pdbName)[line[23:27].strip() + "," + line[18:20].strip()] = coordinatesDict
                        #print(atomsDNA.get(pdbName))
                        enter_coord = False

                if ((list[0].__contains__("ATOM") or list[0].__contains__("HETATM"))
                    ...
                    #print("RNA")
                    if (
                         #    (line[18:20].strip().strip().__eq__("G") or line[18:20].strip().__eq__("DG") or line[
                         #                                                                                18:20].strip().__eq__(
                         #    "GM") or
                         # line[18:20].strip().__eq__("FL") or line[18:20].strip().__eq__("OG") or line[
                         #                                                                         18:20].strip().__eq__(
                         #        "F2") or line[17:20].strip().__eq__("GF0") or line[18:20].strip().__eq__("CG") or line[18:20].strip().__eq__("0G") ) and
                         #
                            ( line[13:15].strip() == ("N1") or line[13:15].strip() == ("N2") or line[13:15].strip() == (
                    "N7") or  line[13:15].strip() == ("N3") or  line[13:15].strip() == ("N6") or line[13:15].strip() == ("N9") or line[13:15].strip() == ("N4")
                            or line[13:15].strip() == ("O6") or line[13:15].strip() == ("O4") or line[13:15].strip() == ("O2") ) and (list[0].__contains__("ATOM") or (list[0].__contains__("HETATM") and (line[18:20].strip().__eq__("F2") or line[18:20].strip().__eq__("FL") ) ))):
...
def addHeptads(key,atoms, max_bond_distance, tetrads):
    distanceDict = {}
    #print("------",distanceDict)
    for keys in atoms.keys():
        ...
        for key in guanines:
                key = str(key)+",DG"
                #print("5555555555",atoms)
                g_n2 = np.array([float(atoms[keys][key]["N2"][0]), float(atoms[keys][key]["N2"][1]),
                              float(atoms[keys][key]["N2"][2])])
                g_n3 = np.array([float(atoms[keys][key]["N3"][0]), float(atoms[keys][key]["N3"][1]),
                              float(atoms[keys][key]["N3"][2])])

                for key2 in atoms[keys]:
                    if key2.__contains__("A"): # or key2.__contains__("C") or key2.__contains__("T")

                        #print(atoms[keys],key2,keys)
                        #exit(0)

                        a_n7 = np.array([float(atoms[keys][key2]["N7"][0]), float(atoms[keys][key2]["N7"][1]),
                                       float(atoms[keys][key2]["N7"][2])])
                        a_n6 = np.array([float(atoms[keys][key2]["N6"][0]), float(atoms[keys][key2]["N6"][1]),
                                      float(atoms[keys][key2]["N6"][2])])


                        a_n1 = np.array([float(atoms[keys][key2]["N1"][0]), float(atoms[keys][key2]["N1"][1]),
                                       float(atoms[keys][key2]["N1"][2])])

                        a_n9 = np.array([float(atoms[keys][key2]["N9"][0]), float(atoms[keys][key2]["N9"][1]),
                                       float(atoms[keys][key2]["N9"][2])])

                        if (key is not key2):

                            N3_N6 = g_n3 - a_n6 # first combination
                            N2_N7 = g_n2 - a_n7

                            N2_N1 = g_n2 - a_n1 # second combination
                            N3_N9 = g_n3 - a_n9


                            if (np.linalg.norm(N3_N6) <=max_bond_distance and np.linalg.norm(N2_N7) <= max_bond_distance):
                                # print(np.linalg.norm(N2_N7))
                                # print(np.linalg.norm(N1_O6))
                                distanceDict.get(keys)["," + key + "," + key2 + ",N2_N7"] = N2_N7
                                distanceDict.get(keys)["," + key + "," + key2 + ",N3_N6"] = N3_N6

                            if (np.linalg.norm(N2_N1) <=max_bond_distance and np.linalg.norm(N3_N9) <= max_bond_distance):

                                distanceDict.get(keys)["," + key + "," + key2 + ",N3_N9"] = N3_N9
                                distanceDict.get(keys)["," + key + "," + key2 + ",N2_N1"] = N2_N1

                    if key2.__contains__("T") and not key2.__contains__("TU") : # there is TU in hetatm line in 5ob3 and no O2 in Thymine of 5ccw
                        #print(key2,"////////////",atoms[keys][key2])
                        t_o4 = np.array([float(atoms[keys][key2]["O4"][0]), float(atoms[keys][key2]["O4"][1]),
                                       float(atoms[keys][key2]["O4"][2])])
                        t_o2 = np.array([float(atoms[keys][key2]["O2"][0]), float(atoms[keys][key2]["O2"][1]),
                                        float(atoms[keys][key2]["O2"][2])])
                        t_n3 = np.array([float(atoms[keys][key2]["N3"][0]), float(atoms[keys][key2]["N3"][1]),
                                        float(atoms[keys][key2]["N3"][2])])

                        if (key is not key2):

                            N2_O4 = g_n2 - t_o4 # first combination
                            N3_N3 = g_n3 - t_n3

                            N2_O2 = g_n2 - t_o2 # second combination
                            N3_N3 = g_n3 - t_n3


                            if (np.linalg.norm(N2_O4) <=max_bond_distance and np.linalg.norm(N3_N3) <= max_bond_distance):
                                # print(np.linalg.norm(N2_N7))
                                # print(np.linalg.norm(N1_O6))
                                distanceDict.get(keys)["," + key + "," + key2 + ",N2_O4"] = N2_O4
                                distanceDict.get(keys)["," + key + "," + key2 + ",N3_N3"] = N3_N3

                            if (np.linalg.norm(N2_O2) <=max_bond_distance and np.linalg.norm(N3_N3) <= max_bond_distance):

                                distanceDict.get(keys)["," + key + "," + key2 + ",N2_O2"] = N2_O2
                                distanceDict.get(keys)["," + key + "," + key2 + ",N3_N3"] = N3_N3

                    if key2.__contains__("C"):
                        c_n4 = np.array([float(atoms[keys][key2]["N4"][0]), float(atoms[keys][key2]["N4"][1]),
                                         float(atoms[keys][key2]["N4"][2])])
                        c_n3 = np.array([float(atoms[keys][key2]["N3"][0]), float(atoms[keys][key2]["N3"][1]),
                                         float(atoms[keys][key2]["N3"][2])])
                        c_o2 = np.array([float(atoms[keys][key2]["O2"][0]), float(atoms[keys][key2]["O2"][1]),
                                         float(atoms[keys][key2]["O2"][2])])

                        if (key is not key2):

                            N3_N4 = g_n3 - c_n4  # first combination
                            N2_N3 = g_n2 - c_n3

                            N3_N4 = g_n3 - c_n4  # second combination
                            N2_O2 = g_n2 - c_o2

                            if (np.linalg.norm(N3_N4) <= max_bond_distance and np.linalg.norm(N2_N3) <= max_bond_distance):
                                # print(np.linalg.norm(N2_N7))
                                # print(np.linalg.norm(N1_O6))
                                distanceDict.get(keys)["," + key + "," + key2 + ",N3_N4"] = N3_N4
                                distanceDict.get(keys)["," + key + "," + key2 + ",N2_N3"] = N2_N3

                            if (np.linalg.norm(N3_N4) <= max_bond_distance and np.linalg.norm(N2_O2) <= max_bond_distance):
                                distanceDict.get(keys)["," + key + "," + key2 + ",N3_N4"] = N3_N4
                                distanceDict.get(keys)["," + key + "," + key2 + ",N2_O2"] = N2_O2


    #print("---22---",distanceDict)

    return distanceDict


def addHeptads_2(key,atoms, max_bond_distance, tetrads):
    distanceDict = {}
    #print("------",distanceDict)
    for keys in atoms.keys():
        distanceDict[keys] = {}
        ...
        for key2 in list_of_keys:
            ...
            if key2.__contains__("A"): # or key2.__contains__("C") or key2.__contains__("T")
                ...
                if (key is not key2):
                    ...
                    for key in guanines:
                        ...
                        # skip = False
                        # for key2 in tetrads_bonds_dict.keys():
                        #     key_2_elements = key2.split(",")
                        #     # print(key_2_elements, "-",key_number,",N2")
                        #     if (key_2_elements[0] == str(key_number) and ("N2" in key_2_elements[1].split("_"))):
                        #         skip = True
                        #         # print(key_2_elements,"***")
                        #
                        # if (skip):
                        #     continue
                        ...
                        if ( np.linalg.norm(N2_N1) <=max_bond_distance  ) :
                            #distanceDict.get(keys)["," + key + "," + key2 + ",N2_N1"] = N2_N1
                            acceptor_content.append(N2_N1)
                            acceptor_name.append("N2_N1")
                            acceptor_1 = True
                            last_key.append(key)


                        if (np.linalg.norm(N2_N7) <= max_bond_distance ):
                            #distanceDict.get(keys)["," + key + "," + key2 + ",N2_N7"] = N2_N7
                            acceptor_content.append(N2_N7)
                            acceptor_name.append("N2_N7")
                            acceptor_2 = True
                            last_key.append(key)

                        if ( np.linalg.norm(N3_N9) <= max_bond_distance):
                            #distanceDict.get(keys)["," + key + "," + key2 + ",N3_N9"] = N3_N9
                            acceptor_content.append(N3_N9)
                            acceptor_name.append("N3_N9")
                            acceptor_3 = True
                            last_key.append(key)

                        if (np.linalg.norm(N3_N6) <=max_bond_distance):
                            #distanceDict.get(keys)["," + key + "," + key2 + ",N3_N6"] = N3_N6
                            acceptor_content.append(N3_N6)
                            acceptor_name.append("N3_N6")
                            acceptor_4 = True
                            last_key.append(key)

                        # if ((acceptor_2 and acceptor_3) or (acceptor_1 and acceptor_3) or (acceptor_1 and acceptor_2) or (acceptor_1 and acceptor_4
                        # or (acceptor_4 and acceptor_2) or (acceptor_4 and acceptor_3))):
                        #     print(acceptor_content)
                        #     print(acceptor_name)
                        #     distanceDict.get(keys)["," + last_key[0] + "," + key2 + "," + acceptor_name[0]] = \
                        #     acceptor_content[0]
                        #     distanceDict.get(keys)["," + last_key[1] + "," + key2 + "," + acceptor_name[1]] = \
                        #     acceptor_content[1]
                        #     break
                        ...
                        or (acceptor_4 and acceptor_2) or (acceptor_4 and acceptor_3))):
                            #print(acceptor_content)
                            #print(acceptor_name, "////")
                            size_of_bonds = len(acceptor_name)

                            for a in range(size_of_bonds):

                                # print(atoms)
                                guanine_plane_normal = getNormal(atoms[keys][last_key[a]]["N1"],
                                                                 atoms[keys][last_key[a]]["N2"],
                                                                 atoms[keys][last_key[a]]["N7"])
                                bond_vector = acceptor_content[a]
                                angle = findAngleBetween(guanine_plane_normal, bond_vector)
                                # print(key2)
                                # print(angle)
                                #print("," + last_key[a] + "," + key2 + "," + acceptor_name[a])
                                if (angle > 50 and angle < 130):
                                    distanceDict.get(keys)["," + last_key[a] + "," + key2 + "," + acceptor_name[a]] = \
                                        acceptor_content[a]
                                    distanceDict.get(keys)["," + last_key[a] + "," + key2 + "," + acceptor_name[a]] = \
                                        acceptor_content[a]
                    ...
                    #     distanceDict.get(keys)["," + key + "," + key2 +"," +donor_name] = donor_content
                    #     distanceDict.get(keys)["," + key + "," + key2 + "," +acceptor_name] = acceptor_content
                    #     continue

            if key2.__contains__("T"): # or key2.__contains__("C") or key2.__contains__("T")
                ...
                if (key is not key2):
                        ...
                        for key in guanines:
                            ...
                            if ( np.linalg.norm(N2_O4) <=max_bond_distance ):
                                #distanceDict.get(keys)["," + key + "," + key2 + ",N2_N1"] = N2_N1
                                acceptor_content.append(N2_O4)
                                acceptor_name.append("N2_O4")
                                acceptor_1 = True
                                last_key.append(key)
                                #print(key, ",",key2,"o4")

                            if (np.linalg.norm(N2_O2) <= max_bond_distance ):
                                #distanceDict.get(keys)["," + key + "," + key2 + ",N2_N7"] = N2_N7
                                acceptor_content.append(N2_O2)
                                acceptor_name.append("N2_O2")
                                acceptor_2 = True
                                last_key.append(key)

                                #print(key, ",", key2,"o2")

                            if ( np.linalg.norm(N3_N3) <= max_bond_distance):
                                #distanceDict.get(keys)["," + key + "," + key2 + ",N3_N9"] = N3_N9
                                acceptor_content.append(N3_N3)
                                acceptor_name.append("N3_N3")
                                acceptor_3 = True
                                last_key.append(key)
                            ...
                            if ((acceptor_2 and acceptor_3) or (acceptor_1 and acceptor_3) or (
                                    acceptor_1 and acceptor_2)):
                                #print(acceptor_content)
                                #print(acceptor_name, "////")
                                size_of_bonds = len(acceptor_name)
                                ...
                                for a in range(size_of_bonds):
                                    #print(atoms)
                                    guanine_plane_normal = getNormal(atoms[keys][last_key[a]]["N1"], atoms[keys][last_key[a]]["N2"], atoms[keys][last_key[a]]["N7"])
                                    bond_vector = acceptor_content[a]
                                    angle = findAngleBetween(guanine_plane_normal, bond_vector)
                                    #print(key2)
                                    #print(angle)
                        ...
                        #
                        # if ( (acceptor_2 and acceptor_3) or (acceptor_1 and acceptor_3)):
                        #     distanceDict.get(keys)["," + key + "," + key2 +"," +acceptor_name[0]] = acceptor_content[0]
                        #     distanceDict.get(keys)["," + key + "," + key2 + "," +acceptor_name[1]] = acceptor_content[1]
                        #     acceptor_content = []
                        #     acceptor_name = []
                        #     acceptor_1 = False
                        #     acceptor_2 = False
            ...
            if key2.__contains__("C"):  # or key2.__contains__("C") or key2.__contains__("T")
                ...
                if (key is not key2):
                    ...
                    for key in guanines:
                        ...
                        if (np.linalg.norm(N3_N4) <= max_bond_distance):
                            acceptor_content.append(N3_N4)
                            acceptor_name.append("N3_N4")
                            acceptor_1 = True
                            last_key.append(key)
                            print("**")

                        if (np.linalg.norm(N2_N3) <= max_bond_distance):
                            acceptor_content.append(N2_N3)
                            acceptor_name.append("N2_N3")
                            acceptor_2 = True
                            last_key.append(key)
                        ...
                        if ((acceptor_2 and acceptor_3) or (acceptor_1 and acceptor_3) or (acceptor_1 and acceptor_2)):
                            #print(acceptor_content)
                            #print(acceptor_name,"////")
                            size_of_bonds = len(acceptor_name)
...
def calculateDistances_AngleMethod(atoms, max_bond_distance):
    ...
    for keys in atoms.keys():
        distanceDict[keys] = {}
        #print("33333333333",keys, distanceDict)

        for key in atoms[keys]:
            if key.__contains__("G") :
                #print(atoms[keys], key, keys)
        ...
        for key in angleDict[keys]:
            ...
            for tetrad_Guanines in tetrads[keys]:
                ...
                if (int(line[1]) in tetrad_Guanines and not (int(line[3]) in tetrad_Guanines)
                    ...
                    tetrads_bonds[keys][i].append(line)
            ...
            if ( (any(("," + line[3] + ",DG," + line[1] + ",DG," + next_bond) in x for x in angleDict[keys].keys()) and
                ...
                tetrads_bonds[keys].append([line])

    return tetrads, tetrads_bonds
